...
class ControllerPaymentEpay extends Controller {
    ...
    public function index() {
        ...
        $str_pad = (int)$this->session->data['order_id'] < 999 ? "5" : "0";
...
//file_put_contents('epay.txt', print_r(PHP_EOL,true),FILE_APPEND);
        if (is_array($result)) {
			$str_pad = (int)$result['ORDER_ORDER_ID'] > 999 ? "5" : "0";
...
file_put_contents('epay.txt', print_r("POSTLINK1",true),FILE_APPEND);			
...
file_put_contents('epay.txt', print_r("POSTLINK2",true),FILE_APPEND);			
file_put_contents('epay.txt', print_r($vs,true),FILE_APPEND);
file_put_contents('epay.txt', print_r(PHP_EOL,true),FILE_APPEND);			
           $vso = $this->$model->addVisaToOrder($vs, $visa['order_id']);
file_put_contents('epay.txt', print_r("POSTLINK3",true),FILE_APPEND);			
file_put_contents('epay.txt', print_r($vso,true),FILE_APPEND);
file_put_contents('epay.txt', print_r(PHP_EOL,true),FILE_APPEND);			
			$this->model_checkout_order->addOrderHistory($visa['order_id'], $visa['transaction_status']);
file_put_contents('epay.txt', print_r("POSTLINK4",true),FILE_APPEND);			
file_put_contents('epay.txt', print_r($visa,true),FILE_APPEND);
file_put_contents('epay.txt', print_r(PHP_EOL,true),FILE_APPEND);				
        } else {
