...
class ControllerSaleVisaKkb extends Controller {
    ...
    public function index() {
        ...
        $this->response->setOutput($this->load->view('sale/visa_kkb', $data));
    }
    ...
    public function refresh() {
        ...
        foreach ($orders as $order) {
            ...
            $request['ORDER_ID'] = str_pad($order['order_id'], 6, "4", STR_PAD_LEFT);
            ...
            if ($kkb->ecode > 0) {
                ...
                $xml = '<merchant id="' . $this->config->get('epay_merchant_id' . (string) $mode) . '"><order id="' . str_pad($order['order_id'], 6, "4", STR_PAD_LEFT) . '"/></merchant>';
                if (strpos($xml, "[RERROR]") > 0) {
                    ...
                    if (is_array($result)) {
                        ...
                        if (in_array("ERROR", $result)) {
                            ...
                            $visa = array(
                                'order_id' => ltrim($result['ORDER_ID'], '4'),
                ...
                if (strpos($xml, "[RERROR]") > 0) {
                    ...
                    if (is_array($result)) {
                        ...
                        if (in_array("ERROR", $result)) {
                            ...
                            $visa = array(
                                'order_id' => ltrim($result['PAYMENT_ORDERID'], '4'),
...
//$visa_kkbID = (isset ($this->request->get['visa_id'])) ? (int) $this->request->get['visa_id'] : NULL;
        ...
        $this->response->setOutput($this->load->view('sale/visa_kkb_info', $data));
    }
