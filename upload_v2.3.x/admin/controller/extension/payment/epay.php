...
class ControllerPaymentEpay extends Controller {
    ...
    public function index() {
		...
		$link = version_compare(VERSION, '2.3.0', '>=') ? "extension/extension" : "extension/payment";
        ...
        $data['breadcrumbs'][] = array(
			'href' => $this->url->link($extension . 'payment/epay', 'token=' . $this->session->data['token'], $ssl),            
            'text' => $this->language->get('heading_title'),
        );

        $data['action'] = $this->url->link($extension . 'payment/epay', 'token=' . $this->session->data['token'], $ssl);
		...
		if (isset($this->request->post['epay_order_prefix'])) {
            $data['epay_order_prefix'] = $this->request->post['epay_order_prefix'];
        } else {
            $data['epay_order_prefix'] = $this->config->get('epay_order_prefix');
        }
        ...
        $this->response->setOutput($this->load->view($extension . 'payment/epay' . $tpl, $data));
    }
